...
def test_band_over_confidence_for_all_types():
    b = Bands(act=0.6, escalate=0.3)
    assert band(NoulAnswer(0.9), b) is Band.ACT          # conf 0.8
    assert band(NoulAnswer(0.1), b) is Band.ACT          # conf 0.8 (a clear no is also ACT)
    ...
    assert band(NoulAnswer(0.8), b) is Band.ACT          # conf 0.6 == act -> inclusive


def test_default_bands_increase_with_severity():
    ...
    assert band(NoulAnswer(0.75), r) is Band.ACT          # read may act at p=0.75
    assert band(NoulAnswer(0.75), d) is not Band.ACT      # delete may not
...
def test_consistent_and_check_pairs():
